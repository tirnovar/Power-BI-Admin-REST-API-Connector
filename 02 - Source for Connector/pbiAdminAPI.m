...
shared pbiAdminAPI.Navigation = () as table =>
    ...
    let
        objects = #table(
            ...
            {
                {
                    Extension.LoadString("GroupsAdmin"), "GroupsAdmin", GroupsNavigation(), "Folder", "GroupsAdmin",
                    false
                },
                {
...
MetricsNavigation = () as table =>
    let
        objects = #table(
            ...
            {
                {
                    Extension.LoadString("Scorecards"), "Scorecards", pbiAdminAPI.Scorecards, "Function", "Scorecards",
                    true
                },
...
pbiAdminAPI = [
    ...
    Authentication = [Aad = [AuthorizationUri = authorize_uri, Resource = resource_uri]],
    Label = Extension.LoadString("DataSourceLabel")
];

// ** Groups
[DataSource.Kind = "pbiAdminAPI"] pbiAdminAPI.Groups = () =>
    let
        apiCall = Json.Document(
            Web.Contents(
                ...
                [
                    ...
                    Query = [#"$top" = "5000", #"$expand" = "users,reports,dashboards,datasets,dataflows,workbooks"]
                ]
            )
        ),
        output = #table(
...
//** Datasets
//**** Table of Datasets
[DataSource.Kind = "pbiAdminAPI"] pbiAdminAPI.Datasets = () =>
    let
        apiCall = Json.Document(
            Web.Contents(api_uri, [RelativePath = "admin/datasets", Headers = [#"Content-Type" = "application/json"]])
        ),
        output = #table(
            type table [
                id = text,
...
[DataSource.Kind = "pbiAdminAPI"] pbiAdminAPI.RefreshesOfAllDatasets = () =>
    let
...
RefreshesOfDatasetType = type function (
    ...
    Documentation.Examples = {[Code = "=pbiAdminAPI.RefreshesOfDataset(""xxx-xxxx-yyxa..."")", Result = ""]}
];
                    ...
                    output = #table(
                        ...
                        List.Transform(
                            ...
                            each
                                ...
                                in
                                    {
                                        _[requestId]?, _[id]?, _[refreshType]?, stTime, enTime, _[status]?,
                                        durationOfRefresh
                                    }
                        )
                    )
...
ParametersOfDatasetType = type function (
    ...
    Documentation.Examples = {[Code = "=pbiAdminAPI.ParametersOfDataset(""xxx-xxxx-yyxa..."")", Result = ""]}
];
...
DatasourcesOfDatasetType = type function (
    ...
    Documentation.LongDescription = "Returns datasources that are used in dataset. 
        !!! THIS FUNCTION IS NOT WORKING WITH DATASET CONNECTED TO SQL, ORACLE, TERADATA, AND SAP HANA IN DIRECTQUERY MODE !!!",
    Documentation.Examples = {[Code = "=pbiAdminAPI.DatasourcesOfDataset(""xxx-xxxx-yyxa..."")", Result = ""]}
];
...
[DataSource.Kind = "pbiAdminAPI"] pbiAdminAPI.DatasetsRefreshables = () =>
    let
        apiCall = Json.Document(
            Web.Contents(
                api_uri,
    ...
    in
        output;
//** Reports
[DataSource.Kind = "pbiAdminAPI"] pbiAdminAPI.Reports = () =>
    let
        apiCall = Json.Document(
            Web.Contents(api_uri, [RelativePath = "admin/reports", Headers = [#"Content-Type" = "application/json"]])
        ),
        output = #table(
            type table [
                id = text,
    ...
    let
        ...
        output = #table(
            ...
            List.Transform(
                ...
                each
                    {
                        _[id]?, _[name]?, DateTime.From(_[lastUpdate]?), _[description]?, _[publishedBy]?, _[workspaceId]?
                    }
            )
        )
    in
...
[DataSource.Kind = "pbiAdminAPI"] pbiAdminAPI.DeploymentPipelines = () =>
    let
        apiCall = Json.Document(
            Web.Contents(
                api_uri,
...
//** Dashboards
//**** Dashboard
[DataSource.Kind = "pbiAdminAPI"] pbiAdminAPI.Dashboards = () =>
    let
        apiCall = Json.Document(
            Web.Contents(
                api_uri, [RelativePath = "admin/dashboards", Headers = [#"Content-Type" = "application/json"]]
            )
        ),
        output = #table(
            type table [
            ...
            List.Transform(
                apiCall[value],
                each {_[id]?, _[displayName]?, _[isReadOnly]?, _[webUrl]?, _[embedUrl]?, _[subscriptions]?}
            )
        )
    in
        output;
//**** Tiles
[DataSource.Kind = "pbiAdminAPI"] pbiAdminAPI.Tiles = () =>
    let
        ...
        unitor = List.RemoveNulls(List.Union(transformator)),
        output = #table(
            type table [id = text, embedUrl = text, rowSpan = text, colSpan = text, reportId = text, datasetId = text],
            List.Transform(unitor, each {_[id]?, _[embedUrl]?, _[rowSpan]?, _[colSpan]?, _[reportId]?, _[datasetId]?})
        )
    in
        output;
//** Dataflows
[DataSource.Kind = "pbiAdminAPI"] pbiAdminAPI.Dataflows = () =>
    let
        apiCall = Json.Document(
            Web.Contents(api_uri, [RelativePath = "admin/dataflows", Headers = [#"Content-Type" = "application/json"]])
        ),
        output = #table(
            type table [
                objectId = text,
...
[DataSource.Kind = "pbiAdminAPI"] pbiAdminAPI.StorageAccounts = () =>
    let
        apiCall = Json.Document(
            Web.Contents(
                api_uri, [RelativePath = "dataflowStorageAccounts", Headers = [#"Content-Type" = "application/json"]]
            )
        ),
        output = #table(
    ...
    in
        output;
//** Gateways
[DataSource.Kind = "pbiAdminAPI"] pbiAdminAPI.Gateways = () =>
    let
        apiCall = Json.Document(
            Web.Contents(
...
//** About Tenant
//**** Capacities
[DataSource.Kind = "pbiAdminAPI"] pbiAdminAPI.Capacities = () =>
    let
        apiCall = Json.Document(
            Web.Contents(
                api_uri, [RelativePath = "admin/capacities", Headers = [#"Content-Type" = "application/json"]]
            )
        ),
        output = #table(
            type table [
    ...
    in
        output;
//**** Encryption Keys
[DataSource.Kind = "pbiAdminAPI"] pbiAdminAPI.TenantKeys = () =>
    let
        apiCall = Json.Document(
            Web.Contents(
                api_uri, [RelativePath = "admin/tenantKeys", Headers = [#"Content-Type" = "application/json"]]
            )
        ),
        output = #table(
            type table [
    ...
    in
        output;
//**** Imports
[DataSource.Kind = "pbiAdminAPI"] pbiAdminAPI.Imports = () =>
    let
        apiCall = Json.Document(
            Web.Contents(api_uri, [RelativePath = "admin/imports", Headers = [#"Content-Type" = "application/json"]])
        ),
        output = #table(
            type table [
                id = text,
    ...
    in
        output;
//**** Avaiable Features
[DataSource.Kind = "pbiAdminAPI"] pbiAdminAPI.AvaiableFeatures = () =>
    let
        apiCall = Json.Document(
            Web.Contents(
                api_uri, [RelativePath = "availableFeatures", Headers = [#"Content-Type" = "application/json"]]
            )
        ),
        output = #table(
...
[DataSource.Kind = "pbiAdminAPI"] pbiAdminAPI.LinkShared = () =>
    let
        apiCall = Json.Document(
            Web.Contents(
                api_uri,
        ...
        generator = List.Buffer(
            List.Generate(
                () => [result = apiCall, uri = result[ArtifactAccessEntities]?],
                each [uri] <> null,
                each
                    [
    ...
    in
        output;
//**** Unused Artifacts
[DataSource.Kind = "pbiAdminAPI"] pbiAdminAPI.UnusedArtifacts = () =>
    let
            ...
            each
                let
                    ...
                    interGenerator = List.Buffer(
                        List.Generate(
                            () => [result = interApiCall, uri = result[continuationUri]?],
                            each [uri] <> null,
                            each
                                [
                                    result = Json.Document(
                                        Web.Contents(uri, [Headers = [#"Content-Type" = "application/json"]])
                                    ),
                                    uri = _[result][continuationUri]?
                                ],
...
[DataSource.Kind = "pbiAdminAPI"] pbiAdminAPI.PublishedToWeb = () =>
    let
        apiCall = Json.Document(
            Web.Contents(
                api_uri,
        ...
        functionVarTester =
            if userIdentifier <> null and userIdentifier <> "" then
                let
                    ...
                    apiCall = Json.Document(
                        Web.Contents(api_uri, [RelativePath = path, Headers = [#"Content-Type" = "application/json"]])
                    ),
                    generator = List.Buffer(
                        List.Generate(
                            () => [result = apiCall, uriReturn = result[continuationUri]?],
                            each [uriReturn] <> null,
                            each
                                [
                                    result = Json.Document(
                                        Web.Contents(uriReturn, [Headers = [#"Content-Type" = "application/json"]])
                                    ),
...
ScorecardsType = type function (
    ...
    Documentation.Examples = {[Code = "=pbiAdminAPI.Scorecards(""xxx-xxxx-yyxa..."")", Result = ""]}
];
...
GoalsType = type function (
    ...
    Documentation.Examples = {[Code = "=pbiAdminAPI.Goals(""xxx-xxxx-yyxa..."",""xxx-xxxx-yyxa..."")", Result = ""]}
];
...
ScannerAPIInfoType = type function (
    ...
    Documentation.Examples = {[Code = "=pbiAdminAPI.ScannerAPIInfo(""xxx-xxxx-yyxa..."")", Result = ""]}
];
...
ScannerAPIInfo = (optional workspaceId as text, optional listWithWokspaceIds as list) =>
    let
        functionVarTester =
            ...
            then
                let
                    apiCall = (wsId as text) => Json.Document(
                        Web.Contents(
...
ScannerAPIStatusAndResultType = type function (
    ...
    Documentation.Examples = {[Code = "=pbiAdminAPI.ScannerAPIStatusAndResultType(""xxx-xxxx-yyxa..."")", Result = ""]}
];
